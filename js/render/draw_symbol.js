...
function drawSymbol(painter, layer, posMatrix, tile, bucket, elementGroups, prefix, sdf, iconsNeedLinear) {
    ...
    var shaderInterfaceName = prefix === 'text' ? 'glyph' : 'icon';
    ...
    bucket.bindBuffers(shaderInterfaceName, gl);
    ...
    if (sdf) {
        ...
        if (layer.paint[prefix + '-halo-width']) {
            ...
            for (var j = 0; j < elementGroups.length; j++) {
                ...
                bucket.setAttribPointers(shaderInterfaceName, gl, shader, group.vertexOffset, layer);
